# Originally named 'npc health tracker.py'
# Define libraries
...
def load_save():
    ...
    try:
        ...
        for data_split in content:
            t_name = data_split[0]
            t_max_hp = int(data_split[1])
            t_current_hp = int(data_split[2])
            if len(npc_list) < 23: # Place in first column
                temp_npc = NPC(f'{t_name}', t_max_hp, ac_var.get(), existing_npc_frame)
                temp_npc.hp_current = t_current_hp
                temp_npc.update_label()
                npc_list.append(temp_npc)
                temp_npc.pack_widgets(len(npc_list)-1, 0)
            elif len(npc_list) < 46: # Place in second column
                temp_npc = NPC(f'{t_name}', t_max_hp, ac_var.get(), existing_npc_frame_2)
                temp_npc.hp_current = t_current_hp
                temp_npc.update_label()
                npc_list.append(temp_npc)
                temp_npc.pack_widgets(len(npc_list)-24, 0)
...
## Inventory functions
inventory_window = None # pylint: disable=invalid-name, redefined-outer-name
